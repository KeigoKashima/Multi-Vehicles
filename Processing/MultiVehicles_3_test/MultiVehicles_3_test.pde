...
public float PrismaticJointForce = 10.0;//可動方向へ動く速度
...
void setup() {
        ...
        //JointTestオブジェクトを作成
        //test = new JointTest(width/2, 0); 
        
        test = new ArrayList<JointTest>();
        //NUM個の一輪Robotを用意して，リストに格納する
        for (int i=0; i<NUM; i++) {
                JointTest t =  new JointTest(5*BodyWidth*(i)/4+30, height/2);
                test.add(t);
        }
}
...
void draw() {
        ...
        for (JointTest t : test) t.display();
        ...
        ////マウスを押したら
        //if(mousePressed){
        //       for (Robot car : cars) car.Tegotae(); 
        //}
